...
### 2026-05-01 — Personal Stats Dashboard + Trip Sharing Scope Proposal
- **Decision file:** `.squad/decisions/inbox/mikey-stats-trip-sharing.md`
- **Audit result:** Foundation is solid. Ride logs, trips, sharing infrastructure all exist. What's missing is career-level stats UI and sharing UX.
- **MVP scope:** Stats aggregation logic (client-side), personal stats page UI, share button + modal, public trip view polish. Total ~17h.
- **Data already exists:** All ride logs in `users/{uid}/rideLogs/{id}`, trip stats in `users/{uid}/trips/{tripId}/stats`, share index in `sharedTrips/{shareId}`.
- **Trip stats computed:** `totalRides`, `totalWaitMinutes`, `parksVisited`, `uniqueAttractions`, `favoriteAttraction` — recomputed on every ride log add via `updateTripStats()`.
- **Current dashboard:** Basic 4-card summary (total rides, parks visited, trips logged, wait minutes). No filtering, no trends, no interactivity.
- **No schema changes needed for MVP.** All data is queryable now.
- **Sharing already 90% done:** `generateShareLink()` exists, `shareId` field on Trip, share index collection exists. Just needs UI: button, modal, link copy.
- **Phase 2 (future):** Sparklines, achievements, park deep-dives, PDF export, social cards (og:meta), crowd comparison. Defer these pending design + time availability.
- **Open questions for Devin:** (1) Time saved calculation — synthetic or crowd-based? (2) Social cards for Twitter/FB? (3) Specific achievement badges?
...
## Sprint Kickoff: Stats Dashboard + Trip Sharing (2026-05-01T18:28:49Z)

**Decision:** D13: Personal Stats Dashboard + Trip Sharing Scope  
**Phase:** Sprint A (MVP)

**Team Assignments:**
- **Mikey:** Decision proposal (COMPLETED) — scoped MVP vs Phase 2, documented audit findings, opened questions for stakeholder
- **Data:** Career stats aggregation (COMPLETED) — built `career-stats.ts` with client-side computations (totalRides, averageWaitTime, mostVisitedPark, etc.)
- **Mouth:** Share UI components (COMPLETED) — ShareModal.tsx, public trip view polish, integrated to trip detail page
- **Stef:** Testing suite (IN PROGRESS) — writing unit + integration + E2E tests for aggregation logic and share flows

**MVP Scope (17 hours):**
1. Stats aggregation logic (Data: 2h) ✅ DONE
2. Personal stats page UI (Mouth: 4h) — pending
3. Share button + modal (Mouth: 3h) ✅ DONE
4. Public trip view polish (Mouth: 2h) ✅ DONE
5. Date range filter (Mouth: 2h) — pending
6. Testing (Stef: 4h) — in progress

**Schema:** No new collections. Reuse `users/{uid}/rideLogs`, `users/{uid}/trips`, `sharedTrips/{shareId}`.

**Next:** Stef completes test suite → Mouth wires stats page → demo + stakeholder review.
