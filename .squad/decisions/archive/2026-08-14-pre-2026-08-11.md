# Archived Canonical Decisions (before 2026-08-11)

## Scope and Provenance

This file preserves the canonical decision material archived during the
2026-08-14 merge. Its source entries predate 2026-08-11. Current decisions
D39–D65 live only in `.squad/decisions.md`.
...
## Archival Provenance: Former Unprocessed Directives
...
**Archived provenance:** D48 is the current canonical deployment requirement and
supersedes D12's policy and process wording. This record retains only the unique
historical and troubleshooting context from the original incident.

After a difficult session recovering from a missing Git remote, the team recorded:
- Git remote `origin` points to `https://github.com/Giffdev/theme-park-wait-time.git`
- Branch: `master` (tracking `origin/master`)
- `npx vercel --prod` hung during that troubleshooting session
- If `git remote -v` is empty, re-add the remote before pushing checked-in work

The incident's inference that pushing `master` automatically deploys is not a
current capability and must not be used as release instruction. Follow D48:
deploy the reviewed commit from a clean, detached worktree, never from the dirty
checkout, then smoke-test https://theme-park-wait-times.vercel.app.
