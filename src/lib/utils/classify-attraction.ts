...
/**
 * Manual overrides for well-known attractions (partial name match).
 * Mirrors scripts/attraction-overrides.ts for client-side fallback.
 */
const ATTRACTION_OVERRIDES: Record<string, AttractionType> = {
  // Thrill
...
/**
 * Client-side classification of an attraction based on name and entityType.
 * Used as a fallback when `attractionType` is not populated in Firestore.
 */
export function classifyAttraction(name: string, entityType?: string): AttractionType {
  ...
  // 3. entityType fallback
  if (entityType === 'SHOW') return 'show';
  if (entityType === 'MEET_AND_GREET') return 'character-meet';
